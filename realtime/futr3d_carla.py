...
class FUTR3DInference:
    def __init__(self, config_path, ckpt_path, score_threshold=0.3):
        ...
        # Build pipeline — skip GT-only steps
        skip = {"LoadAnnotations3D", "ObjectRangeFilter",
                "ObjectNameFilter", "PointsRangeFilter"}
    ...
    def infer(self, lidar_pts, images_bgr, lidar2img_list, cam2lidar_list):
        ...
        data = dict(
            points=lidar_pts,
            img=images_bgr,
            img_filename=[f"cam_{i}" for i in range(len(images_bgr))],
